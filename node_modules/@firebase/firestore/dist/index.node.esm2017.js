    ...
    async write(batch, userCallback) {
        ...
        try {
            ...
            if (e.name === 'IndexedDbTransactionError') {
                ...
                logError(LOG_TAG$3, 'Dropping write that cannot be persisted: ' + e);
                userCallback.reject(new FirestoreError(Code.UNAVAILABLE, 'Failed to persist write: ' + e));
                return;
            }
            else {
                throw e;
    ...
    async synchronizeQueryViewsAndRaiseSnapshots(targets) {
        ...
        for (const targetId of targets) {
            ...
            if (queries && queries.length !== 0) {
                ...
                debugAssert(this.isPrimary === true, 'A secondary tab should never have an active target without an active query.');
...
const LOG_TAG$5 = 'WatchChangeAggregator';
/**
...
class LruScheduler {
    ...
    scheduleGC(localStore) {
        ...
        this.gcTask = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection" /* LruGarbageCollection */, delay, () => {
            this.gcTask = null;
            this.hasRun = true;
            return localStore
...
class SimpleDb {
    ...
    static openOrCreate(name, version, schemaConverter) {
        ...
        logDebug(LOG_TAG$7, 'Opening database:', name);
        return new PersistencePromise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                logDebug(LOG_TAG$7, 'Database "' + name + '" requires upgrade from version:', event.oldVersion);
                const db = event.target.result;
                schemaConverter
                    .createOrUpgrade(db, request.transaction, event.oldVersion, SCHEMA_VERSION)
                    .next(() => {
                    logDebug(LOG_TAG$7, 'Database upgrade to version ' + SCHEMA_VERSION + ' complete');
                });
            };
        }).toPromise();
    }
    ...
    static delete(name) {
        logDebug(LOG_TAG$7, 'Removing database:', name);
    ...
    async runTransaction(mode, objectStores, transactionFn) {
        ...
        while (true) {
            ...
            try {
                ...
                logDebug(LOG_TAG$7, 'Transaction failed with error: %s. Retrying: %s.', error.message, retryable);
                if (!retryable) {
...
class SimpleDbTransaction {
    ...
    abort(error) {
        ...
        if (!this.aborted) {
            logDebug(LOG_TAG$7, 'Aborting transaction:', error ? error.message : 'Client-initiated abort');
            this.aborted = true;
...
class SimpleDbStore {
    ...
    put(keyOrValue, value) {
        let request;
        if (value !== undefined) {
            logDebug(LOG_TAG$7, 'PUT', this.store.name, keyOrValue, value);
            request = this.store.put(value, keyOrValue);
        }
        else {
            logDebug(LOG_TAG$7, 'PUT', this.store.name, '<auto-key>', keyOrValue);
     ...
     */
    add(value) {
        logDebug(LOG_TAG$7, 'ADD', this.store.name, value, value);
    ...
    get(key) {
        ...
        return wrapRequest(request).next(result => {
            ...
            logDebug(LOG_TAG$7, 'GET', this.store.name, key, result);
            return result;
        });
    }
    delete(key) {
        logDebug(LOG_TAG$7, 'DELETE', this.store.name, key);
     ...
     */
    count() {
        logDebug(LOG_TAG$7, 'COUNT', this.store.name);
    ...
    deleteAll(indexOrRange, range) {
        logDebug(LOG_TAG$7, 'DELETE ALL', this.store.name);
 ...
 * limitations under the License.
 */
const LOG_TAG$8 = 'IndexedDbPersistence';
/**
...
class IndexedDbPersistence {
    ...
    updateClientMetadataAndTryBecomePrimary() {
        return this.simpleDb
            ...
            logDebug(LOG_TAG$8, 'Releasing owner lease after error during lease refresh', e);
    ...
    canActAsPrimary(txn) {
        ...
        return store
            ...
            .next(canActAsPrimary => {
            if (this.isPrimary !== canActAsPrimary) {
                logDebug(LOG_TAG$8, `Client ${canActAsPrimary ? 'is' : 'is not'} eligible for a primary lease.`);
            }
    ...
    releasePrimaryLeaseIfHeld(txn) {
        ...
        return store.get(DbPrimaryClient.key).next(primaryClient => {
            if (this.isLocalClient(primaryClient)) {
                logDebug(LOG_TAG$8, 'Releasing primary lease.');
    ...
    isClientZombied(clientId) {
        try {
            const isZombied = this.webStorage.getItem(this.zombiedClientLocalStorageKey(clientId)) !==
                null;
            logDebug(LOG_TAG$8, `Client '${clientId}' ${isZombied ? 'is' : 'is not'} zombied in LocalStorage`);
            return isZombied;
        }
        catch (e) {
            // Gracefully handle if LocalStorage isn't working.
            logError(LOG_TAG$8, 'Failed to get zombied client id.', e);
            return false;
        }
    }
    /**
 ...
 * limitations under the License.
 */
const LOG_TAG$a = 'PersistentStream';
...
class PersistentStream {
    ...
    handleStreamClose(error) {
        ...
        logDebug(LOG_TAG$a, `close with error: ${error}`);
        this.stream = null;
        ...
        return (fn) => {
            this.queue.enqueueAndForget(() => {
                if (this.closeCount === startCloseCount) {
                    ...
                    logDebug(LOG_TAG$a, 'stream callback skipped by getCloseGuardedDispatcher.');
                    return Promise.resolve();
                }
            });
        };
...
class FirestoreClient {
    ...
    start(componentProvider, persistenceSettings) {
        ...
        this.credentials.setChangeListener(user => {
            if (!initialized) {
                initialized = true;
                logDebug(LOG_TAG$b, 'Initializing. user=', user.uid);
    ...
    handleCredentialChange(user) {
        ...
        logDebug(LOG_TAG$b, 'Credential Changed. Current user: ' + user.uid);
...
class UserDataReader {
    constructor(serializer, preConverter) {
        this.serializer = serializer;
        this.preConverter = preConverter;
    }
    /** Parse document data from a non-merge set() call. */
    parseSetData(methodName, input) {
        const context = new ParseContext(0 /* Set */, methodName, FieldPath.EMPTY_PATH);
    ...
    parseMergeData(methodName, input, fieldPaths) {
        const context = new ParseContext(2 /* MergeSet */, methodName, FieldPath.EMPTY_PATH);
        validatePlainObject('Data must be an object, but it was:', context, input);
        const updateData = this.parseObject(input, context);
        let fieldMask;
        let fieldTransforms;
    ...
    parseUpdateData(methodName, input) {
        const context = new ParseContext(1 /* Update */, methodName, FieldPath.EMPTY_PATH);
        validatePlainObject('Data must be an object, but it was:', context, input);
    ...
    parseQueryValue(methodName, input, allowArrays = false) {
        const context = new ParseContext(allowArrays ? 4 /* ArrayArgument */ : 3 /* Argument */, methodName, FieldPath.EMPTY_PATH);
    ...
    parseSentinelFieldValue(value, context) {
        ...
        if (value instanceof DeleteFieldValueImpl) {
            ...
            const numericIncrement = new NumericIncrementTransformOperation(this.serializer, operand);
    ...
    parseArrayTransformElements(methodName, elements) {
        return elements.map((element, i) => {
            ...
            const context = new ParseContext(3 /* Argument */, methodName, FieldPath.EMPTY_PATH);
...
class GrpcConnection {
    ...
    openStream(rpcName, token) {
        ...
        grpcStream.on('error', (grpcError) => {
            logDebug(LOG_TAG$d, 'GRPC stream error. Code:', grpcError.code, 'Message:', grpcError.message);
            const code = mapCodeFromRpcCode(grpcError.code);
